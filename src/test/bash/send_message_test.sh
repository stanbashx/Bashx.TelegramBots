...
"${SCRIPT}" -b "${TGBOTS_BOT_ID}" -bss "${TGBOTS_BOT_SECRET_SRC}" > "${STDOUT}" 2> "${STDERR}"
. $asserts/strings/eq.sh "${SCRIPT}" "$?" '1'
. $asserts/files/empty.sh "${STDOUT}"
...
TGBOTS_CHAT_ID=''
TGBOTS_BOT_SECRET="${TGBOTS_BOT_SECRET}" \
 "${SCRIPT}" -b "${TGBOTS_BOT_ID}" -bss "${TGBOTS_BOT_SECRET_SRC}" -c "${TGBOTS_CHAT_ID}" > "${STDOUT}" 2> "${STDERR}"
. $asserts/strings/eq.sh "${SCRIPT}" "$?" '1'
. $asserts/files/empty.sh "${STDOUT}"
...
TGBOTS_MESSAGE=''
TGBOTS_BOT_SECRET="${TGBOTS_BOT_SECRET}" \
 "${SCRIPT}" -b "${TGBOTS_BOT_ID}" -bss "${TGBOTS_BOT_SECRET_SRC}" -c "${TGBOTS_CHAT_ID}" -m "${TGBOTS_MESSAGE}" > "${STDOUT}" 2> "${STDERR}"
. $asserts/strings/eq.sh "${SCRIPT}" "$?" '1'
. $asserts/files/empty.sh "${STDOUT}"
...
for VALUE in "${VALUES[@]}"; do
 ...
 PATH="${mocks}/curl/bin:${PATH}" \
  ...
  TGBOTS_BOT_SECRET="${TGBOTS_BOT_SECRET}" \
  "${SCRIPT}" -b "${TGBOTS_BOT_ID}" -bss "${TGBOTS_BOT_SECRET_SRC}" -c "${TGBOTS_CHAT_ID}" -m "${TGBOTS_MESSAGE}" -h 200 > "${STDOUT}" 2> "${STDERR}"
 . $asserts/strings/eq.sh "${SCRIPT}" "$?" '1'
 . $asserts/files/empty.sh "${STDOUT}"
...
PATH="${mocks}/curl/bin:${PATH}" \
 ...
 TGBOTS_BOT_SECRET="${TGBOTS_BOT_SECRET}" \
 "${SCRIPT}" -b "${TGBOTS_BOT_ID}" -bss "${TGBOTS_BOT_SECRET_SRC}" -c "${TGBOTS_CHAT_ID}" -m "${TGBOTS_MESSAGE}" -h 200 -d "${TGBOTS_DST}" -C 'true' > "${STDOUT}" 2> "${STDERR}"
